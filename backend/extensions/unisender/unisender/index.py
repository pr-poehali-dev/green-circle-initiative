...
def handle_send(body: dict) -> dict:
    ...
    Supports multiple recipients via 'to_emails' array.
    """
    ...
    # Validation
    if not to_emails:
        return cors_response(400, {"error": "to_email or to_emails is required"})
    ...
    for email in to_emails:
        ...
        if "status" in result and result["status"] == "error":
            ...
            results.append({"email": email, "job_id": result.get("job_id")})

    if failed_count == len(to_emails):
        return cors_response(400, {"error": "All emails failed to send"})
