...
def handle_callback(event: dict, origin: str) -> dict:
    ...
    print(f"[DEBUG] handle_callback started")
    print(f"[DEBUG] event keys: {list(event.keys())}")
    
    # Parse body
    body_str = event.get('body', '{}')
    print(f"[DEBUG] body_str length: {len(body_str) if body_str else 0}")
    print(f"[DEBUG] body_str: {body_str[:200] if body_str else 'EMPTY'}...")
    
    if event.get('isBase64Encoded'):
        body_str = base64.b64decode(body_str).decode('utf-8')
        print(f"[DEBUG] decoded body_str: {body_str[:200]}...")
    ...
    print(f"[DEBUG] from payload - code: {code[:20] if code else 'EMPTY'}..., code_verifier: {code_verifier[:20] if code_verifier else 'EMPTY'}..., device_id: {device_id[:20] if device_id else 'EMPTY'}...")
    ...
    if not code:
        ...
        print(f"[DEBUG] from query - code: {code[:20] if code else 'EMPTY'}..., code_verifier: {code_verifier[:20] if code_verifier else 'EMPTY'}..., device_id: {device_id[:20] if device_id else 'EMPTY'}...")

    if not code:
        print(f"[ERROR] No code provided")
        return error(400, 'Authorization code is required', origin)

    if not code_verifier:
        print(f"[ERROR] No code_verifier provided")
    ...
    try:
        ...
        print(f"[DEBUG] Calling exchange_code_for_token...")
        ...
        if 'error' in token_data:
            print(f"[ERROR] VK token error: {token_data}")
            return error(400, token_data.get('error_description', 'VK auth failed'), origin)

        vk_access_token = token_data.get('access_token')
        print(f"[DEBUG] Got access_token: {vk_access_token[:20] if vk_access_token else 'NONE'}...")

        # Get user info from VK ID API
        print(f"[DEBUG] Calling get_vk_user_info...")
        user_info = get_vk_user_info(vk_access_token, client_id)
        print(f"[DEBUG] user_info keys: {list(user_info.keys()) if user_info else 'EMPTY'}")
        print(f"[DEBUG] user_info: {json.dumps(user_info)[:300]}...")
        ...
        print(f"[DEBUG] Extracted: vk_user_id={vk_user_id}, name={full_name}, email={vk_email}")
        
        if not vk_user_id:
            print(f"[ERROR] No vk_user_id in user_info")
            return error(400, 'Failed to get user ID from VK', origin)
        ...
        try:
            print(f"[DEBUG] Starting DB operations...")
            cur = conn.cursor()
            now = datetime.now(timezone.utc).isoformat()
            print(f"[DEBUG] now={now}")

            # Cleanup expired tokens periodically
            print(f"[DEBUG] Cleaning up expired tokens...")
            cleanup_expired_tokens(cur, S)

            # Check if user exists by vk_id (parameterized query)
            print(f"[DEBUG] Checking if user exists with vk_id={vk_user_id}...")
            cur.execute(
            ...
            if row:
                ...
                print(f"[DEBUG] Existing user_id={user_id}, updating last_login...")
                # Update last login
                cur.execute(
                    f"UPDATE {S}users SET last_login_at = %s, updated_at = %s WHERE id = %s",
                    (now, now, user_id)
                )
                print(f"[DEBUG] Updated last_login for user_id={user_id}")
            else:
                print(f"[DEBUG] Creating new user with vk_id={vk_user_id}...")
                # Create new user
                cur.execute(
                    f"""INSERT INTO {S}users
                        (vk_id, email, name, avatar_url, email_verified, password_hash, created_at, updated_at, last_login_at)
                ...
                email = vk_email
                name = full_name
                print(f"[DEBUG] Created new user_id={user_id}")

            # Create tokens
            print(f"[DEBUG] Creating JWT tokens for user_id={user_id}...")
            ...
            print(f"[DEBUG] Tokens created, storing refresh_token...")
            ...
            print(f"[DEBUG] Refresh token stored, committing...")
            ...
            print(f"[DEBUG] Sending success response with user_id={user_id}")
            return response(200, response_data, origin)

        except Exception as e:
            conn.rollback()
            print(f"[ERROR] Database exception: {type(e).__name__}: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        print(f"[ERROR] HTTPError: {error_body}")
        return error(500, 'VK API error', origin)
    except Exception as e:
        print(f"[ERROR] Top-level exception: {type(e).__name__}: {str(e)}")
        import traceback
        traceback.print_exc()
