...
(function () {
    ...
    const isProduction = () => {
        ...
        return parent == window; // Default to production if no dev indicators found
    };
    ...
    function createPoehaliiBadge() {
        ...
        const projectMetaTag = document.querySelector('meta[name="pp-name"]');

        let projectName = "";
        if (projectMetaTag) {
            projectName = projectMetaTag.getAttribute('value');
        } else {
        ...
        badge.src = 'https://cdn.poehali.dev/intertnal/img/created-badge.svg';
        badge.style.position = 'fixed';
        badge.style.bottom = '20px';
        badge.style.right = '20px';
        badge.style.width = '10rem';
        ...
        text.textContent = 'Сайт создан на Poehali.dev';
        ...
        tooltip.textContent = "Подключите премиум, чтобы скрыть";
    ...
    // ───── Фильтр «failed to connect to websocket» от Vite ─────
    function isViteWebSocketError(text) {
        text = text.toLowerCase();
        return text.includes('[vite]') && text.includes('failed to connect to websocket');
    }
    ...
    console.error = function (...args) {
        ...
        // ► игнорируем шум перезагрузки WebSocket
        if (isViteWebSocketError(errorText)) return;
    ...
    function extractImportantErrorInfo(errorMessage) {
        ...
        const N = 140; // For example, keep first 10 words
        return errorMessage.split(' ').slice(0, N).join(' ');
    }
    ...
    function showErrorScreen(errorMessage, filePath) {
        ...
        if (existingOverlay) {
            ...
            lastError = simplifiedError; // Keep full error in storage, but display simplified version
        ...
        message.textContent = simplifiedError;
